    ...
    if (tmppath.empty()) {
      ...
      for (size_t i = 0; i < dbnum_; i++) {
        std::string childpath =
          kc::strprintf("%s%cmr-%04x-%04x-%08x-%03d%ckct",
                        tmppath.c_str(), kc::File::PATHCHR, pid, tid, ts,
                        (int)(i + 1), kc::File::EXTCHR);
...
private:
   ...
   */
  struct CacheComparator {
    kc::LexicalComparator comp;          ///< lexical comparator
    /** comparing operator */
    bool operator()(char* const & abuf, char* const& bbuf) {
      uint64_t aksiz;
      char* akbuf = abuf + kc::readvarnum(abuf, sizeof(aksiz), &aksiz);
      uint64_t bksiz;
      char* bkbuf = bbuf + kc::readvarnum(bbuf, sizeof(bksiz), &bksiz);
  ...
  struct MergeLine {
    ...
    kc::Comparator* comp;                ///< lexical comparator
  ...
  bool flush_cache() {
    ...
    while (!err && cit != citend) {
      char* rbuf = *cit;
      uint64_t ksiz;
      const char* kbuf = rbuf + kc::readvarnum(rbuf, sizeof(ksiz), &ksiz);
      const char* vbuf = kbuf + ksiz;
      uint64_t vsiz;
      size_t step = kc::readvarnum(vbuf, sizeof(vsiz), &vsiz);
